...
import { 
  ...
  X, Users, Heart, Send, MessageCircle
} from "lucide-react";
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black animate-in slide-in-from-bottom duration-500 flex flex-col max-w-[420px] mx-auto overflow-hidden">
      ...
      <div className="relative z-20 px-6 pb-24 space-y-4">
        <div 
          ref={scrollRef}
          className="h-48 overflow-y-auto no-scrollbar space-y-3 mask-fade-top"
        >
          {messages.map((msg) => (
            <div key={msg.id} className="flex flex-col gap-0.5 animate-in fade-in slide-in-from-left-2 duration-300">
              ...
              <p className="text-[11px] text-white/90 bg-black/30 backdrop-blur-sm px-3 py-1.5 rounded-xl rounded-tl-none border border-white/5 inline-block max-w-[80%]">
                {msg.text}
              </p>
            </div>
          ))}
        ...
        <form onSubmit={handleSendMessage} className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full h-12 bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl pl-10 pr-4 text-xs text-white focus:outline-none focus:border-primary/50 transition-all placeholder:text-white/20"
            />
          </div>
          <button 
            type="submit"
            className="h-12 w-12 bg-primary rounded-2xl flex items-center justify-center text-black shadow-[0_0_20px_rgba(204,255,0,0.3)]"
          >
