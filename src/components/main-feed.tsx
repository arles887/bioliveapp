    ...
    <div className="flex flex-col w-full h-full p-8 space-y-12 pb-44 overflow-y-auto no-scrollbar animate-in fade-in slide-in-from-bottom-8 duration-1000">
      ...
      <div className="flex gap-5 overflow-x-auto no-scrollbar py-2">
        <div className="flex flex-col items-center gap-3 shrink-0 group cursor-pointer">
          <div className="w-20 h-20 rounded-[2rem] bg-primary flex items-center justify-center text-black shadow-[0_0_25px_rgba(204,255,0,0.3)] animate-float transition-all group-hover:scale-110">
            <Plus size={32} strokeWidth={3} />
          </div>
          <span className="text-[10px] font-black uppercase tracking-widest text-primary/80">My Bio</span>
        </div>
        {feedItems.filter(i => i.isLive).map(live => (
          <div key={live.id} onClick={() => setActiveLiveId(live.id)} className="flex flex-col items-center gap-3 shrink-0 cursor-pointer group">
            <div className="w-20 h-20 rounded-[2rem] p-[3px] bg-gradient-to-tr from-primary to-green-500 relative">
              <div className="absolute inset-0 rounded-[2rem] animate-pulse-glow bg-primary/20 -z-10"></div>
              <div className="w-full h-full rounded-[1.8rem] overflow-hidden relative grayscale group-hover:grayscale-0 transition-all duration-500">
      ...
      <div className="space-y-10">
        <div className="flex items-center justify-between px-2">
            <h2 className="text-xs font-black uppercase tracking-[0.5em] text-white/30 italic">Global Transmission</h2>
        ...
        {feedItems.map((item) => (
          <div 
            ...
            className="group relative w-full overflow-hidden rounded-[2.5rem] bg-card border border-white/5 transition-all duration-700 hover:neon-border hover:shadow-[0_0_50px_rgba(204,255,0,0.15)] cursor-pointer"
          >
            <div className={cn(
              "relative",
              item.type === "reel" ? "aspect-[4/5]" : "aspect-[16/11]"
            )}>
              <Image 
                src={item.thumbnail} 
                fill 
              ...
              <div className="absolute top-8 left-8 flex gap-3">
                {item.type === "live" ? (
                  <Badge className="bg-primary text-black border-none px-5 py-2 rounded-xl text-[10px] font-black tracking-widest uppercase italic">
                    LIVE NOW
                  </Badge>
                ) : (
                  <Badge className="bg-white/10 backdrop-blur-xl text-white border-white/20 px-5 py-2 rounded-xl text-[10px] font-black tracking-widest uppercase">
                    TRANSMISSION
                  </Badge>
                )}
              </div>
              ...
              <div className="absolute bottom-10 left-10 right-10">
                <div className="flex flex-col gap-3 text-white">
                  <div className="flex items-center gap-3 mb-2">
                    <div className="w-10 h-10 rounded-[1.2rem] bg-primary/20 flex items-center justify-center text-primary border border-primary/20 group-hover:bg-primary group-hover:text-black transition-all">
                      <Zap size={16} fill="currentColor" />
                    </div>
                    <span className="text-xs font-black uppercase tracking-[0.3em] text-primary">@{item.user}</span>
                  </div>
                  <h3 className="text-3xl font-black leading-tight tracking-tighter group-hover:text-primary transition-colors italic uppercase">{item.title}</h3>
                  <div className="flex items-center gap-8 mt-6 text-[11px] font-black uppercase tracking-[0.2em] text-white/30">
                    {item.type === "live" ? (
                      <span className="flex items-center gap-2 text-primary/80"><Eye size={16} /> {item.views} SIGNAL</span>
                    ) : (
                      <>
                        <span className="flex items-center gap-2 hover:text-primary transition-colors"><Heart size={16} /> {item.likes}</span>
                        <span className="flex items-center gap-2"><Play size={16} /> {item.views}</span>
                      </>
                    )}
                  </div>
                </div>
              ...
              {item.type === "reel" && (
                <div className="absolute right-10 bottom-10 flex flex-col gap-6">
                  <div className="p-5 rounded-[1.8rem] glass-dark text-white hover:bg-primary hover:text-black transition-all shadow-2xl hover:scale-110">
                    <Heart size={28} />
                  </div>
                  <div className="p-5 rounded-[1.8rem] glass-dark text-white hover:bg-primary hover:text-black transition-all shadow-2xl hover:scale-110">
